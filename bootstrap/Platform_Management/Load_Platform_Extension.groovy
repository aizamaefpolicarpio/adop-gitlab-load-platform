...
if [ $PLATFORM_EXTENSION_TYPE = "aws"  ]; then
...
elif [ $PLATFORM_EXTENSION_TYPE = "docker" ]; then

	if [ -d ${WORKSPACE}/service/docker ]; then
	
		if [ -f ${WORKSPACE}/service/docker/docker-compose.yml ]; then
		
				echo "#######################################"
				echo "Adding Docker-Compose configurations"
